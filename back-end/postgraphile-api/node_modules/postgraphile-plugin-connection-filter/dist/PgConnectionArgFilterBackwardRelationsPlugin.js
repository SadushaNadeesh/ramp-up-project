...
const PgConnectionArgFilterBackwardRelationsPlugin = (builder, { pgSimpleCollections }) => {
        ...
        for (const spec of backwardRelationSpecs) {
            ...
            if (isOneToMany) {
                if (!omit(foreignTable, "many")) {
                    ...
                    addField(fieldName, `Filter by the object’s \`${fieldName}\` relation.`, FilterManyType, makeResolveMany(spec), spec, `Adding connection filter backward relation field from ${describePgEntity(table)} to ${describePgEntity(foreignTable)}`);
                ...
                addField(fieldName, `Filter by the object’s \`${fieldName}\` relation.`, ForeignTableFilterType, resolveSingle, spec, `Adding connection filter backward relation field from ${describePgEntity(table)} to ${describePgEntity(foreignTable)}`);
