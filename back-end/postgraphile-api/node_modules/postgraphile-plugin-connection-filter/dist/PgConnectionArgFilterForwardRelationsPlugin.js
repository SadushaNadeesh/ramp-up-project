      ...
      exists(
        ...
        for (const spec of forwardRelationSpecs) {
            ...
            if (!ForeignTableFilterType)
                continue;
            addField(fieldName, `Filter by the object’s \`${fieldName}\` relation.`, ForeignTableFilterType, resolve, spec, `Adding connection filter forward relation field from ${describePgEntity(table)} to ${describePgEntity(foreignTable)}`);
